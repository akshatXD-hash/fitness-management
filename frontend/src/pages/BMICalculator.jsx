...
export const BMICalculator = () => {
  ...
  const renderInsights = (text) => {
    if (!text) return <p className="text-muted-foreground">No intelligent insights generated for this combination.</p>;
      ...
      <div className="space-y-3 mt-4">
        {lines.map((line, idx) => {
          ...
          if (colonIdx > -1 && colonIdx < 40) {
            ...
            return (
              <div key={idx} className="bg-card/60 p-4 rounded-sm border border-border/50 flex gap-4 hover:border-emerald-primary/50 transition-colors group shadow-sm inline-flex w-full items-start">
                 <div className="mt-0.5"><Zap className="h-4 w-4 text-emerald-primary group-hover:scale-110 transition-transform" /></div>
                 <div className="w-full">
                    <span className="font-black text-foreground tracking-widest uppercase text-[10px] block mb-1 opacity-70 group-hover:opacity-100 transition-opacity">{label}</span>
                    <span className="text-sm text-muted-foreground font-medium leading-relaxed">{value}</span>
                 </div>
              </div>
            );
          }
          ...
          return (
            <div key={idx} className="bg-card/30 p-4 rounded-sm border-l-2 border-emerald-primary/30 text-sm text-muted-foreground flex gap-3 items-start">
    ...
    return (
      <div className="mt-8 mb-4 flex flex-col items-center relative">
        <div className="relative w-[300px] h-[150px] overflow-hidden drop-shadow-xl">
           ...
           <div className="absolute top-[35px] left-[35px] w-[230px] h-[230px] bg-card rounded-full shadow-inner border border-border/30"></div>
           ...
           >
              ...
              <div className="w-1 h-[100px] bg-card mx-auto shadow-md"></div>
           </div>
           
           {/* Needle anchor pin */}
           <div className="absolute bottom-[-10px] left-1/2 -translate-x-1/2 w-5 h-5 bg-card rounded-full z-30 shadow-lg border-4 border-border"></div>
        </div>

        {/* Legend */}
        <div className="flex justify-between w-[320px] text-[10px] font-black text-muted-foreground uppercase tracking-widest mt-4">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
      <Card>
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-2xl font-black text-foreground tracking-tight">BMI Predictor</h2>
        </div>
      ...
      {result && result.bmi ? (
        <Card className="bg-card border-emerald-primary/50 shadow-[0_0_30px_rgba(16,185,129,0.05)] relative overflow-hidden">
          ...
          <div className="flex justify-between items-start mb-2 relative z-10">
            <div>
              <h2 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Calculated State</h2>
        ...
        <div className="h-full min-h-[300px] border-2 border-dashed border-border flex flex-col items-center justify-center p-8 text-center bg-card/30 rounded-sm">
           ...
           <p className="text-muted-foreground font-medium max-w-sm">Input your physiological metrics to fire the AI engine and receive your custom body mass breakdown.</p>
        </div>
      )}
    </div>
  );
